...
const StaffSeeRegistrations = () => {
    ...
    try {
      ...
      const jobsWithStudents = await Promise.all(
        drives.map(async (job) => {
          const studentsResponse = await axios.get(`${BASE_URL}/staff/registeredstudents/${job.id}`, { withCredentials: true });
          console.log(`Registered Students Response for job ${job.id}:`, studentsResponse.data);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setLoading(true);
      await axios.delete(`${BASE_URL}/staff/job/${jobToDelete.jobId}`, { withCredentials: true });
      fetchJobs();
    } catch (err) {
  ...
  const exportToExcel = (students, companyName, mode = "all") => {
    ...
    if (mode === "your" && currentStaffEmail) {
      exportStudents = students.filter(student => student.staffEmail === currentStaffEmail);
    }
    ...
    const fileName = mode === "your" ? `${companyName}_Your_Students_${timestamp}.xlsx` : `${companyName}_Students_${timestamp}.xlsx`;
    ...
    <div className="p-2 sm:p-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        {filterJobs(jobs).map((job) => (
          <div key={job.id} className="bg-white rounded-lg shadow-md p-3 sm:p-6 hover:shadow-lg transition-shadow overflow-hidden">
            ...
            <div className="space-y-3">
              <div className="flex items-center text-gray-600">
                ...
                <span className="truncate">Drive Date: {job.driveDate || 'N/A'}</span>
              </div>
              <div className="flex items-center text-gray-600">
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-2">
                    {job.students && job.students.length > 0 && (
                      <>
                        <button
                          onClick={() => exportToExcel(job.students, job.companyName, "all")}
                        ...
                        {viewMode === "your" && (
                          <button
                            onClick={() => exportToExcel(job.students, job.companyName, "your")}
                ...
                {job.students && job.students.length > 0 ? (
                  <>
                    <p className="mb-4 text-gray-600 text-center">
                      Showing {filterStudents(job.students).length} out of {viewMode === "your" ? job.students.filter(student => student.staffEmail === currentStaffEmail).length : job.students.length} students
                    </p>
